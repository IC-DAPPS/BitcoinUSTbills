...
<div class="container-wide mx-auto px-6 py-8">
  ...
  {#if error}
    <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
      ...
      <button class="text-red-600 underline mt-2" on:click={() => (error = null)}>
        Dismiss
      </button>
    </div>
  {/if}
            ...
            <tbody>
              {#each transactions as transaction}
                <tr class="bg-white border-b hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {transaction.status === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}">
                      {transaction.status}
                    </span>
                  </td>
                </tr>
            ...
            <div class="transaction-card card p-5">
              <div class="transaction-card-header mb-4">
                <div class="flex justify-between items-start">
                  ...
                  <div class="transaction-status">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {transaction.status === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}">
                      {transaction.status}
                    </span>
                  </div>
                </div>
              ...
              <div class="transaction-card-body">
                <div class="transaction-details-grid">
                  ...
                  <div class="transaction-detail-item">
                    <p class="text-xs text-secondary font-medium uppercase tracking-wide">Currency</p>
                    <p class="text-sm font-semibold text-gray-900">{transaction.currency}</p>
                  </div>
                </div>

                <div class="transaction-detail-item mt-3">
                  <p class="text-xs text-secondary font-medium uppercase tracking-wide">Description</p>
                ...
                <div class="transaction-detail-item mt-3">
                  ...
                  <p class="text-sm font-semibold text-gray-900">{transaction.timestamp}</p>
                </div>
              </div>
            </div>
          {/each}

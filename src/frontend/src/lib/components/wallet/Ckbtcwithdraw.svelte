<script lang="ts">
  ...
  let disabled = $derived<boolean>(
    recipientError !== "" || amountError !== "" || isLoading
  );
...
<Button variant="secondary" class="flex-1" onclick={() => (open = true)}
  >⬆ Withdraw</Button
>
    ...
    <div class="flex gap-3 p-2 rounded-lg border border-gray-300 bg-gray-100">
      ...
      <div>
        ...
        <p class="text-sm text-gray-500">
          Balance: {ckbtcBalance.number ?? "0"} ckBTC
        </p>
      </div>
    </div>

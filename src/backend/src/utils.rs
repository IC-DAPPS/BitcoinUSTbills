...
pub fn validate_email(email: &str) -> Result<()> {
    if email.is_empty() {
        return Err(BitcoinUSTBillsError::validation_error("Email cannot be empty"));
    }
    
    if !email.contains('@') {
        return Err(BitcoinUSTBillsError::validation_error("Invalid email format"));
    }
    ...
    if !domain.contains('.') {
        return Err(BitcoinUSTBillsError::validation_error("Invalid email domain"));
    }
...
pub fn validate_phone_number(phone: &str) -> Result<()> {
    ...
    if cleaned.len() < 10 || cleaned.len() > 15 {
        return Err(BitcoinUSTBillsError::validation_error("Invalid phone number length"));
    }
    
    // Check if starts with + for international numbers
    if cleaned.starts_with('+') {
        if cleaned.len() < 11 {
            return Err(BitcoinUSTBillsError::validation_error("Invalid international phone number"));
        }
    }
...
pub fn parse_dollar_amount(amount_str: &str) -> Result<u64> {
    ...
    if let Ok(amount) = cleaned.parse::<f64>() {
        ...
        Err(BitcoinUSTBillsError::validation_error("Invalid amount format"))
    }
}
...
pub fn generate_unique_id(prefix: &str) -> String {
    ...
    let caller = ic_cdk::msg_caller().to_text();
...
pub fn validate_country(country: &str) -> Result<()> {
    if country.is_empty() {
        return Err(BitcoinUSTBillsError::validation_error("Country is required"));
    }
    
    // Simple validation - country should be 2-3 characters
    if country.len() < 2 || country.len() > 3 {
        return Err(BitcoinUSTBillsError::validation_error("Invalid country code"));
    }
